...
class Parser {
  ...
  async parseURL(feedUrl) {
    const headers = { ...DEFAULT_HEADERS, ...this.options.headers }
    ...
    if (response.headers.get('etag')) {
      this.etags[feedUrl] = response.headers.get('etag')
    }

    if (response.headers.get('last-modified')) {
      this.lastModified[feedUrl] = response.headers.get('last-modified')
    }
    ...
    const parsedData = await this.parseString(text);
    return {
      ...parsedData,
