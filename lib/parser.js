      ...
      let req = get(requestOpts, (res) => {
        if (this.options.maxRedirects && res.statusCode >= 300 && res.statusCode < 400 && res.headers['location']) {
          ...
          if (redirectCount === this.options.maxRedirects) {
            ...
            return this.parseURL(newLocation, null, redirectCount + 1).then(data => resolve({ ...data, finalUrl: newLocation, wasRedirected }), reject);
          }
        ...
        res.on('end', () => {
          if (res.statusCode === 304 || xml.trim() === '') {
            return resolve({
              statusCode: res.statusCode,
              wasRedirected,
              finalUrl: wasRedirected ? url.resolve(feed.url, res.headers['location']) : originalUrl
            })
          };
          this.parseString(xml)
            .then(parsedData => {
              resolve({
                ...
                finalUrl: wasRedirected ? newLocation : originalUrl,
                statusCode: res.statusCode
              })
            })
        });
